...
The RooMUHistos package needs to be built, as it is still development/test stage. \\
...
% data points and statistical errors. If both systematic errors and correlation matrix are provided, a covariance 
...
% The ExpDataSet provides public methods to access reference publication, name of the observable, interaction
...
% are stored in a form of {\it MUH1D} object. Note that systematic uncertainties can be introduced to the MonteCarlo
% analysis by varying one or several parameters in the simulation; in such case the uncertainties will be stored
% as a {\it MuVertBand} data member of the MUH1D object that holds the results. 
...
% These methods are purely virtual: Reset, EndOfRun,and DoIt (which is called by the Analyze method). 
}
