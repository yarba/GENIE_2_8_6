...
#include "validation/Tuning/GExDataABC.h"
#include "validation/Tuning/GExModelABC.h"
#include "validation/Tuning/GFitParamList.h"
#include "validation/Tuning/GFitInputs.h"
...
GFitInputs::GFitInputs(
 GExDataABC * data, GExModelABC *  model, GFitParamList * params) :
fData   (data), 
fModel  (model), 
fParams (params)
{ 
...
//________________________________________________________________________________________
GExModelABC & GFitInputs::Model(void) 
{ 
  return *fModel;       
}
//________________________________________________________________________________________
...
{
  if (option) {
    return fData->Statistic(fModel,*fParams,option);
  }
  return fData->Statistic(fModel,*fParams, NULL);
}
...
// To be used with a TVirtualFitter
  ...
  GExModelABC & model = inputs->Model();

  f = data.Statistic(&model,fp,"likelihood-ratio");
